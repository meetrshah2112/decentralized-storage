...
def download_file_from_provider(provider_node, cid):
    if not provider_node.agent_api_url:
        raise Exception("Provider node does not have an agent API URL.")

    download_url = f"{provider_node.agent_api_url.rstrip('/')}/download/"
    ...
    try:
        ...
        raise Exception(f"Provider Agent download failed: {error}")
